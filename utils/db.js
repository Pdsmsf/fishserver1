...
//通过账号获取玩家和道具数据
exports.get_user_prop_data = function(account,callback){
    callback = callback == null? nop:callback;
    if(account == null){
        callback(null);
        return;
    }
    var sql = 'SELECT U.userid,U.account,U.name,U.headimg,U.lv,U.exp,U.coins,U.vip,U.money,U.gems,U.roomid,U.RenameCount,U.ReHeadCount,P.propId,P.ice FROM t_users U LEFT JOIN t_property P ON U.userid = P.userid WHERE U.account = "' + account + '"';
    console.log(sql);
...
// 创建道具表
exports.create_property = function(userId,callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(false);
        return;
    }
    var sql = 'INSERT INTO t_property(userid,ice) VALUES(' + userId + ',' + 5 + ')';
    console.log(sql);
    //基于异步的事件处理
    query(sql, function(err, rows, fields) {
        if (err) {
...
//道具表是否存在
exports.is_property_exist = function(userId,callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(false);
        return;
    ...
    var sql = 'SELECT userid FROM t_property WHERE userid = "' + userId + '"';
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if (err) {
            throw err;
...
//修改道具数据
exports.set_ice_of_property = function(userId,ice,callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(false);
        return;
    }
    // var sql = 'UPDATE t_property SET ice = '+ ice + ' WHERE userId = "' + userId + '"';
    var sql = 'UPDATE t_property SET ice = ice + ' + ice + ' WHERE userid = ' + userId + ';';
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if (err) {
            throw err;
...
//模块导出函数 NewName是否存在
exports.is_NewName_exist = function(NewName,callback){
    callback = callback == null? nop:callback;
    if(NewName == null){
        callback(false);
        return;
    ...
    var sql = 'SELECT userid FROM t_users WHERE name = "' + NewName + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            throw err;
        }
...
//模块导出函数 修改用户名
exports.set_userName_of_user = function(userId,NewName,gems,callback){
    callback = callback == null? nop:callback;
    if(NewName == null){
        callback(false);
        return;
    }
    NewName = crypto.toBase64(NewName);
    NewName = '"' + NewName + '"';
    // var sql = 'UPDATE t_users SET name = '+ NewName + ' WHERE userId = "' + userId + '"';
    var sql = 'UPDATE t_users SET name = {0},gems = gems - {1},RenameCount = RenameCount + 1 WHERE userId={2}';
    sql = sql.format(NewName,gems,userId);
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if(err){
            console.log(err);
...
//模块导出函数 修改用户头像
exports.set_heading_of_user = function(userId,index,gems,callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(false);
        return;
    }
    // var sql = 'UPDATE t_users SET headimg = '+ index + ' WHERE userId = "' + userId + '"';
    var sql = 'UPDATE t_users SET headimg = {0},gems = gems - {1},ReHeadCount = ReHeadCount + 1 WHERE userId={2}';
    sql = sql.format(index,gems,userId);
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if(err){
            // console.log(err);
...
exports.get_user_data = function(account,callback){
    ...
    var sql = 'SELECT userid,account,name,headimg,lv,exp,coins,vip,gems,roomid,RenameCount,ReHeadCount FROM \
    ...
    query(sql, function(err, rows, fields) {
        ...
        console.log("dbname========================="+rows[0].name);
        rows[0].name = crypto.fromBase64(rows[0].name);
        callback(rows[0]);
    });
};
//模块导出函数 通过userid获取玩家数据
...
// 获取进入房间的信息
exports.get_userinfo_by_userId = function(userId, callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(null);
        return;
...
exports.get_vip_by_userId = function(userId,callback){
    callback = callback == null? nop:callback;
    if(userId == null){
        callback(null);
        return;
    ...
    var sql = 'SELECT vip FROM t_users WHERE userid = "' + userId + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            callback(null);
            throw err;
...
//模块导出函数 通过账号获取金币（游戏币）
exports.coins = function(account,callback){
    callback = callback == null? nop:callback;
    if(account == null){
        callback(null);
        return;
    ...
    var sql = 'SELECT coins FROM t_users WHERE account = "' + account + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            callback(null);
            throw err;
...
exports.create_user = function(account,name,coins,gems,sex,headimg,RenameCount,ReHeadCount,callback){
    callback = callback == null? nop:callback;
    ...
    var sql = 'INSERT INTO t_users(account,name,coins,gems,sex,headimg,power,RenameCount,ReHeadCount) VALUES("{0}","{1}",{2},{3},{4},{5},{6},{7},{8})';
    sql = sql.format(account,name,coins,gems,sex,headimg,0,RenameCount,ReHeadCount);
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if (err) {
            throw err;
...
exports.refush_gems = function(user_id, gem, callback){
    callback = callback == null? nop:callback;
    var sql = 'UPDATE t_users SET gems =  '+ gem + ' WHERE userid = ' + user_id;
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if(err){
            callback(false);
...
exports.refush_power = function (user_id, power, callback) {
    callback = callback == null? nop:callback;
    var sql = 'UPDATE t_users SET power =  '+ power + ' WHERE userid = ' + user_id;
    console.log(sql);
    query(sql, function(err, rows, fields) {
        if(err){
            callback(false);
...
exports.create_room = function(roomId,conf,ip,port,create_time,callback){
    callback = callback == null? nop:callback;
    var baseScore = conf.gamebasescore;
    var sql = "INSERT INTO t_rooms(uuid,id,base_info,ip,port,create_time,baseScore) \
                VALUES('{0}','{1}','{2}','{3}',{4},{5},{6})";
    var uuid = Date.now() + roomId;
    var baseInfo = JSON.stringify(conf);
    sql = sql.format(uuid,roomId,baseInfo,ip,port,create_time,baseScore);
    console.log(sql);
...
exports.update_seat_info = function(roomId,seatIndex,userId,icon,name,callback){
    callback = callback == null? nop:callback;
    var sql = 'UPDATE t_rooms SET user_id{0} = {1},user_score{0} = "{2}",user_name{0} = "{3}" WHERE id = "{4}"';
    name = crypto.toBase64(name);
...
// 获取有空位的房间
exports.get_empty_room = function(baseScore, callback){
    callback = callback == null? nop:callback;
    var sql = 'SELECT id FROM t_rooms WHERE ((user_id0 = 0) OR (user_id1 = 0) OR (user_id2 = 0) OR (user_id3 = 0)) AND baseScore = {0}  ORDER BY create_time;';

    sql = sql.format((parseInt(baseScore)).toString());
